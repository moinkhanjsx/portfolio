...
const Home = () => (
  <section id="home" className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-b from-amber-400 to-amber-100 dark:from-gray-900 dark:to-gray-800 w-full max-w-full px-4 sm:px-6 lg:px-8 relative">
    {/* Animated SVG background */}
    <motion.img 
      src="/vite.svg" 
      alt="Decorative background element" 
      className="absolute top-0 left-0 w-24 sm:w-40 md:w-64 opacity-10 pointer-events-none select-none" 
      style={{zIndex:0}}
      animate={{ rotate: 360 }}
    ...
    <motion.img 
      src="/vite.svg" 
      alt="Decorative background element" 
      className="absolute bottom-0 right-0 w-20 sm:w-32 md:w-52 opacity-10 pointer-events-none select-none" 
      style={{zIndex:0}}
      animate={{ rotate: -360 }}
    ...
    <div 
      className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center relative z-10 px-2"
      role="group"
      aria-label="Main action buttons"
    >
      <motion.a 
        href="#projects" 
        className="px-4 sm:px-6 md:px-8 py-2.5 sm:py-3 md:py-4 bg-amber-600 text-white rounded-xl shadow-xl hover:bg-amber-700 transition text-base sm:text-lg md:text-xl font-bold tracking-wide transform focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2"
      ...
      <motion.a 
        ...
        download 
        className="px-4 sm:px-6 md:px-8 py-2.5 sm:py-3 md:py-4 bg-white text-amber-700 border-2 border-amber-600 rounded-xl shadow-xl hover:bg-amber-50 hover:text-amber-900 transition text-base sm:text-lg md:text-xl font-bold flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2"
        whileHover={{ scale: 1.05, y: -2 }}
        whileTap={{ scale: 0.95 }}
        transition={{ type: "spring", stiffness: 400, damping: 17 }}
    ...
    {/* Social links */}
    <div 
      className="flex gap-4 sm:gap-6 mt-6 sm:mt-8 relative z-10 px-2"
      role="group"
    ...
    >
      <motion.a 
        ...
        className="text-gray-700 dark:text-gray-200 hover:text-amber-600 dark:hover:text-amber-400 transition-colors text-xl sm:text-2xl focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2 rounded p-2" 
        title="GitHub"
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-6 h-6 sm:w-8 sm:h-8" aria-hidden="true">
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 sm:w-8 sm:h-8" aria-hidden="true">
