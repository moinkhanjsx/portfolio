...
// Advanced typing component with multiple texts
const AdvancedTyping = ({ texts, speed = 100, deleteSpeed = 50, pauseTime = 2000 }) => {
...
const WordByWordTyping = ({ text, speed = 200, startDelay = 0 }) => {
    ...
    <span>
      {displayWords.map((word, index) => (
        <span key={index} className="animate-fadeIn">
          {word}{index < displayWords.length - 1 ? ' ' : ''}
        </span>
      ))}
      {hasStarted && currentWordIndex < words.length && <span className="animate-pulse">|</span>}
    </span>
  )
}

// Simple typing component for fallback
